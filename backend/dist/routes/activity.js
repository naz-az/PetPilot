        ...
        const formattedActivities = activities.map(activity => {
            ...
            switch (activity.action) {
                ...
                case 'payment_completed':
                    ...
                    description = `Payment of $${activity.details?.amount || 'N/A'} was processed successfully`;
                    icon = 'card-outline';
                    color = '#007AFF';
                    break;
                case 'review_created':
                    title = 'Review Posted';
                    description = `You left a ${activity.details?.rating || 'N/A'}-star review`;
                    icon = 'star-outline';
                    color = '#FF9500';
                    break;
                case 'medical_record_created':
                    title = 'Medical Record Added';
                    description = `Added medical record: ${activity.details?.title || 'N/A'}`;
                    icon = 'medical-outline';
                    color = '#FF3B30';
                    break;
                case 'pet_created':
                    title = 'Pet Added';
                    description = `Added ${activity.details?.petName || 'new pet'} to your profile`;
                    icon = 'paw-outline';
                    color = '#34C759';
                    break;
...
router.get('/stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const [totalActivities, recentActivities, totalBookings, completedBookings, totalReviews, totalPayments] = await Promise.all([
