...
router.post('/booking/:bookingId', auth_1.authenticateToken, [
    (0, express_validator_1.body)('paymentMethod').isIn(['card', 'apple_pay', 'google_pay', 'paypal']).withMessage('Invalid payment method'),
...
// Request refund
router.post('/:id/refund', auth_1.authenticateToken, [
    (0, express_validator_1.body)('reason').trim().isLength({ min: 1, max: 500 }).withMessage('Refund reason is required and must be less than 500 characters'),
...
router.get('/methods/available', auth_1.authenticateToken, async (req, res) => {
    try {
